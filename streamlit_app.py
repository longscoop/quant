...
from quant.research_ui import (
    ...
    ensure_research_navigation_page,
    render_research_candidate_pool,
    render_research_home,
    render_research_stock_detail,
    render_research_stock_summary,
    research_security_index,
)
from quant.storage import PostgresStore
from quant.ui import empty_state, metric_cards, page_header, render_public_page, research_disclaimer, security_option_label
from quant.workflows import build_factor_run, run_backtest_run, sync_hs300, train_model_run
...
def render_stock_data(store):
    ...
    code = st.selectbox(
        "证券",
        codes,
        index=research_security_index(codes, st.session_state.get("research_selected_security")),
        format_func=lambda value: security_option_label(value, memory.securities[value].name),
    )
    ...
    factor_run, rankings = latest_rankings(store)
        ...
        if existing_position:
            ...
            if st.button("保存组合权重", key=f"stock_save_{code}"):
                try:
                    ...
                    st.warning(str(exc))
    render_research_stock_summary(st, memory, code, factor_run=factor_run, model_run=model_run)
    if ranking:
        score = ranking["score"]
...
def render_factor_research(store):
    ...
    if st.button("构建六类 PIT 因子"):
        run_id = build_factor_run(
            store,
            [day for day in dates if day <= selected],
            CN_DEFAULT_CONTEXT,
            market_config=build_cn_market_config(store),
        )
        run = store.get_run(run_id)
...
def render_model_training(store):
    ...
    if default_date is None:
        ...
        return
    available_dates = sorted({date.fromisoformat(str(row["as_of_date"])) for row in (factor_run.get("payload") or {}).get("rows", [])})
    if len(available_dates) < 3:
        st.warning("因子运行至少需要覆盖三个有序日期区间，才能配置 train/valid/test。")
        return
    valid_index = max(1, int(len(available_dates) * 0.6))
    test_index = min(max(valid_index + 1, int(len(available_dates) * 0.8)), len(available_dates) - 1)
    with st.form("qlib_model_training"):
        train_start = st.date_input("训练开始", available_dates[0])
        train_end = st.date_input("训练结束", available_dates[valid_index - 1])
        valid_start = st.date_input("验证开始", available_dates[valid_index])
        valid_end = st.date_input("验证结束", available_dates[test_index - 1])
        test_start = st.date_input("测试开始", available_dates[test_index])
        test_end = st.date_input("测试结束", available_dates[-1])
        st.caption(
            f"市场 {CN_DEFAULT_CONTEXT.market_id} · 币种 {CN_DEFAULT_CONTEXT.currency} · "
            f"日历 {CN_DEFAULT_CONTEXT.calendar_id} · 股票池 {CN_DEFAULT_CONTEXT.universe_id} · 基准 {CN_DEFAULT_CONTEXT.benchmark_id}"
        )
        submitted = st.form_submit_button("训练 Qlib 模型并生成研究信号", type="primary")
    if submitted:
        split = TimeSplitConfig(
            calendar_id=CN_DEFAULT_CONTEXT.calendar_id,
            train=DateSegment(train_start, train_end),
            valid=DateSegment(valid_start, valid_end),
            test=DateSegment(test_start, test_end),
        )
    ...
    prediction_dates = metadata.get("prediction_dates") or [
        segment.get("end")
        for name, segment in (metadata.get("segments") or {}).items()
        if name == "test" and isinstance(segment, dict)
    ] or (model.get("parameters") or {}).get("prediction_dates") or []
    if not isinstance(prediction_dates, list):
        prediction_dates = [prediction_dates]
    model_details = st.columns(4)
    model_details[0].metric("训练截止日", sanitize_display_value(metadata.get("train_end") or ((metadata.get("segments") or {}).get("train") or {}).get("end")))
    ...
    if context:
        st.caption(
            f"市场 {context.get('market_id')} · 币种 {context.get('currency')} · 日历 {context.get('calendar_id')} · "
            f"股票池 {context.get('universe_id')} · 基准 {context.get('benchmark_id')} · Recorder {metadata.get('recorder_id') or '--'}"
        )
    signal_analysis = metadata.get("signal_analysis") or {}
    diagnostics = st.columns(4)
    diagnostics[0].metric("IC", sanitize_display_value(signal_analysis.get("ic") if signal_analysis.get("ic") is not None else "--"))
    diagnostics[1].metric("ICIR", sanitize_display_value(signal_analysis.get("icir") if signal_analysis.get("icir") is not None else "--"))
    diagnostics[2].metric("RankIC", sanitize_display_value(signal_analysis.get("rank_ic") if signal_analysis.get("rank_ic") is not None else "--"))
    diagnostics[3].metric("RankICIR", sanitize_display_value(signal_analysis.get("rank_icir") if signal_analysis.get("rank_icir") is not None else "--"))
...
def render_backtest_results(store):
    ...
    if st.button("运行历史回测"):
        run_id = run_backtest_run(
            ...
            int(top_n),
            float(cost),
            market_config=build_cn_market_config(store, cost_bps=float(cost)),
        )
        run = store.get_run(run_id)
...
def render_industry_heat(store):
    ...
    page_header(st, "行业观察", "从行业覆盖、研究关注度和相对表现了解市场线索。")
    factor_run = latest_run(store, "factors", "completed")
            ...
            [
                {
                    ...
                    "优先研究标的": row["研究候选数量"],
                    "研究关注度": row["景气标签"],
                    "近期相对表现": format_metric(row["20日相对表现"], percent=True),
                    "中期相对表现": format_metric(row["60日相对表现"], percent=True),
                }
                for row in rows
            ]
        ),
    ...
    st.dataframe(
        frame(
            [
                {"名称": memory.securities[code].name if code in memory.securities else "未知证券", "代码": code, "行业": selected}
                for code in codes
            ]
        ),
        hide_index=True,
...
ensure_research_navigation_page(st.session_state, pages)
page = st.sidebar.radio("研究模块", pages, index=0, key="research_navigation_page")
...
if page == "研究首页":
    render_public_page(st, render_research_home, st, store)
elif page == "候选池":
    render_public_page(st, render_research_candidate_pool, st, store)
elif page == "个股详情":
    render_public_page(st, render_research_stock_detail, st, store)
elif page == "行业观察":
    render_public_page(st, render_industry_heat, store)
elif page == "我的组合":
    # 公开口径：按保存权重模拟持有；实际执行仍由 T/T+1 组合账本计算。
    render_public_page(st, render_portfolio_workbench, st, store)
elif page == "历史验证":
    render_public_page(st, render_historical_validation, st, store)
elif page == "管理员":
    render_admin_workbench(st, store)
else:
    render_public_page(st, render_data_status, store)
