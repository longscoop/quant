...
def _latest_industries(memory) -> dict[str, str]:
    latest = {}
...
def _factor_row_by_code(factor_run, code) -> Mapping | None:
    if _mapping(factor_run).get("status") != "completed":
        return None
    return next((row for row in _run_rows(factor_run, "rankings") if row.get("ts_code") == code), None)
...
def _candidate_date(model_row: Mapping | None, factor_row: Mapping | None) -> str:
    for row in (model_row, factor_row):
        value = _mapping(row).get("as_of_date")
        if _display_date(value) != "未知":
...
def research_candidate_rows(memory, factor_run=None, model_run=None) -> list[dict]:
    ...
    industries = _latest_industries(memory)
    candidates = []
    for index, source_row in enumerate(ordered):
        code = source_row.get("ts_code")
        security = getattr(memory, "securities", {}).get(code)
        factor_row = _factor_row_by_code(factor_run, code) if model_completed else source_row
        ...
        candidates.append({
            ...
            "风险水平": risk_level,
            "数据日期": _candidate_date(source_row if model_completed else None, factor_row),
            "专业详情": _candidate_details(source_row if model_completed else None, factor_row, ordering_source=ordering_source),
        })
    return candidates
