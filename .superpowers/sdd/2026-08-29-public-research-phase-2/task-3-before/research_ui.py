...
from .research import latest_research_snapshot_date, research_candidate_rows, research_data_freshness
...
def _show_candidate_actions(st, store, candidate: dict, joined_codes: set[str]) -> None:
    ...
    with st.container(horizontal=True, gap="small"):
        if st.button("查看详情", key=f"research_home_detail_{candidate_key}", width="stretch"):
            st.session_state[SELECTED_SECURITY_KEY] = code
            st.session_state[TARGET_PAGE_KEY] = "stock_detail"
            _rerun(st)
        if code in joined_codes:
            st.button("已加入", key=f"research_home_add_{candidate_key}", disabled=True, width="stretch")
        elif st.button("加入研究组合", key=f"research_home_add_{candidate_key}", width="stretch"):
            try:
