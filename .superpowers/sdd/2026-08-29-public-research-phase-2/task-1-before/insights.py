...
def research_candidate_rows(memory, factor_run=None, model_run=None):
    """Build homepage/pool rows, preferring completed model output."""
    if model_run and model_run.get("status") == "completed":
        return _model_candidate_rows(memory, model_run)
    if factor_run and factor_run.get("status") == "completed":
        return _factor_candidate_rows(memory, factor_run)
    return []
