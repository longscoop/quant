...
from quant.presentation import format_metric, sanitize_display_rows, sanitize_display_value, status_feedback
...
def refresh_today_opportunities(store):
    ...
    if latest is None:
        raise ValueError("尚无行情数据，请先同步沪深300数据")
...
def render_overview(store):
    ...
    if rankings:
        ...
        st.info("暂无 pit_v1.0 排名。重新同步新版财务/估值数据后，运行“因子研究”生成研究候选。")
    return
...
def render_data_management(store):
    ...
    if sync_runs:
        ...
        if latest_sync["status"] == "running":
            ...
            level = st.success if latest_sync["status"] == "completed" else st.error
            ...
            error_suffix = f" · 原因：{latest_sync.get('error')}" if latest_sync.get("error") else ""
            level(f"最近同步：{latest_sync['status']} · 请求区间 {params.get('start_date', '不可用')} 至 {params.get('end_date', '不可用')} · 行情 {price_count} 条 · 财务 {financial_count} 条 · 估值 {valuation_count} 条{error_suffix}")
    ...
    if st.button("同步沪深300", type="primary"):
        if not token:
            ...
            feedback = st.success if run["status"] == "completed" else st.error
            feedback(f"同步运行 {run_id}：{run['status']} {run['error'] or ''}")
...
def render_stock_data(store):
    ...
    if not codes:
        st.info("暂无真实数据，请先在“数据管理”同步沪深300。")
        return
    ...
    if ranking:
        ...
        st.info("该股票尚无 pit_v1.0 研究摘要；请完成新版数据同步与因子运行。")
    ...
    if not prices:
        ...
        if not ohlc:
            st.warning("当前历史行情缺少开、高、低字段，无法诚实绘制 K 线；请在“数据状态”重新同步行情以补齐 OHLC 数据。")
        else:
...
def render_factor_research(store):
    ...
    if st.button("构建六类 PIT 因子"):
        ...
        run = store.get_run(run_id)
        feedback_level, feedback_message = status_feedback(run, f"因子运行已完成：{run_id}")
    ...
    if not runs:
        ...
        return
    run = st.selectbox("因子运行", runs, format_func=lambda value: f"{value['run_id']} · {value['status']}")
    metadata = run_payload(run).get("metadata") or {}
...
def render_model_training(store):
    ...
    if not factor_runs:
        ...
        return
    factor_run = st.selectbox("因子运行", factor_runs, format_func=lambda value: value["run_id"])
    ...
    if st.button("训练模型并生成研究信号"):
        ...
        run = store.get_run(run_id)
        feedback_level, feedback_message = status_feedback(run, f"模型运行已完成：{run_id}")
        getattr(st, feedback_level)(feedback_message)
    models = [run for run in store.list_runs() if run["run_type"] == "model"]
    if not models:
        st.info("尚无模型运行。下一步：使用覆盖预测日期和更早日期的因子运行。")
        return
    model = st.selectbox("模型运行", models, format_func=lambda value: f"{value['run_id']} · {run_status_summary(value)['label']}")
    ...
    if model["status"] == "completed":
        ...
        st.error(model.get("error") or "模型运行失败")
...
def render_backtest_results(store):
    ...
    if not models:
        st.info("请先完成模型训练，再运行历史回测。")
        return
    model = st.selectbox("模型运行", models, format_func=lambda value: value["run_id"])
    ...
    if st.button("运行历史回测"):
        ...
        run = store.get_run(run_id)
        (st.success if run["status"] == "completed" else st.error)(f"回测运行 {run_id}：{run['status']} {run['error'] or ''}")
    results = [run for run in store.list_runs() if run["run_type"] == "backtest"]
    if not results:
        st.info("尚无回测结果。下一步：选择已完成的模型运行并执行历史回测。")
        return
    result = results[0]
    if result["status"] != "completed":
        st.error(result.get("error") or "回测失败")
        return
...
def render_run_records(store):
    ...
    if runs:
        st.dataframe(frame(runs), use_container_width=True)
    else:
...
def render_today_opportunities(store):
    ...
    if st.button("刷新今日机会", type="primary"):
        try:
            ...
            st.success(f"已保存最新研究快照：{run_id}")
            _safe_rerun(st)
        except ValueError as exc:
            st.warning(str(exc))
    if not candidates:
        empty_state(st, "还没有今日机会", "最新运行没有可展示的排名，或当前数据尚未补齐 PIT 财务/估值字段。", "先在“数据状态”重新同步数据；随后点击“刷新今日机会”仅构建最新交易日快照。")
    else:
...
def render_stock_pool(store):
    ...
    candidates = research_candidate_rows(memory, factor_run=factor_run, model_run=model_run)
    if not candidates:
        empty_state(st, "候选池暂不可用", "需要至少一项已完成的因子运行或模型运行。", "前往“数据状态”完成数据同步，再运行因子构建。")
        research_disclaimer(st)
        return
...
def render_industry_heat(store):
    ...
    if not rows:
        empty_state(st, "行业数据不足", "当前没有可用的行业归属记录。", "请先在“数据状态”同步沪深300数据。")
        research_disclaimer(st)
        return
    st.dataframe(frame(rows), hide_index=True, use_container_width=True)
...
def render_strategy_lab(store):
    ...
    models = [run for run in store.list_runs() if run["run_type"] == "model" and run["status"] == "completed"]
    if not models:
        empty_state(st, "还没有可实验的模型", "策略实验需要一项已完成模型运行。", "先到“数据状态”完成因子构建和模型预测。")
        research_disclaimer(st)
        return
    ...
    if submitted:
        ...
        if run["status"] == "completed":
            st.success(f"实验已保存：{run_id}")
        elif run["status"] == "not_trainable":
            st.warning(run.get("error") or "样本不足，无法完成实验")
        else:
            st.error(run.get("error") or "实验失败")
    st.info("调仓频率固定为月度；模板会记录研究假设，回测只使用真实已有模型分数。")
    experiments = [run for run in store.list_runs() if run["run_type"] == "backtest"]
    if experiments:
        st.markdown("### 最近实验")
        st.dataframe(frame([{**(run.get("parameters") or {}), "状态": run["status"], "运行 ID": run["run_id"]} for run in experiments[:10]]), hide_index=True, use_container_width=True)
    research_disclaimer(st)
...
def render_portfolio(store):
    ...
    if not memory.securities:
        empty_state(st, "证券池为空", "无法创建组合成员。", "先同步沪深300数据。")
        return
    ...
    if positions and st.button("运行我的组合历史回测", type="primary"):
        ...
        run = store.get_run(run_id)
        (st.success if run["status"] == "completed" else st.warning)(f"组合回测 {run_id}：{run['status']} {run.get('error') or ''}")
    ...
    if personal_runs:
        ...
        if personal["status"] == "completed":
            metric_cards(st, [("组合累计收益", format_metric((payload.get("metrics") or {}).get("total_return"), percent=True), "按保存权重买入并持有"), ("相对沪深300", format_metric((payload.get("metrics") or {}).get("excess_return"), percent=True), "共同交易日"), ("最大回撤", format_metric((payload.get("metrics") or {}).get("max_drawdown"), percent=True), "历史样本")])
        else:
            st.warning(personal.get("error") or "组合历史数据不足")
    research_disclaimer(st)
...
def render_data_status(store):
    ...
    metric_cards(st, [("最新行情", str(quality["latest_trade_date"] or "不可用"), "交易日"), ("证券", str(counts.get("securities", 0)), "当前研究范围"), ("行情记录", str(counts.get("prices", 0)), "price_bars"), ("财务记录", str(counts.get("financials", 0)), "已披露记录"), ("估值记录", str(counts.get("valuation_count", 0)), "daily_basic")])
    runs = store.list_runs()
    if runs:
        st.markdown("### 研究状态")
        state_rows = []
        for run_type, label in (("sync", "数据同步"), ("factors", "因子构建"), ("model", "模型预测"), ("backtest", "回测验证")):
            run = next((item for item in runs if item["run_type"] == run_type), None)
            if not run:
                state_rows.append({"阶段": label, "状态": "待执行", "运行 ID": "—"})
            else:
                status = run["status"]
                label_status = "已完成" if status == "completed" else "不可训练" if status == "not_trainable" else "失败" if status == "failed" else "进行中"
                state_rows.append({"阶段": label, "状态": label_status, "运行 ID": run["run_id"]})
        st.dataframe(frame(state_rows), hide_index=True, use_container_width=True)
    with st.expander("数据同步", expanded=False):
        render_data_management(store)
    with st.expander("因子与模型运行", expanded=False):
        render_factor_research(store)
        st.divider()
        render_model_training(store)
    with st.expander("运行记录与同步审计"):
        render_run_records(store)
    with st.expander("研究流程说明"):
        st.markdown("1. 同步沪深300数据。\n2. 构建覆盖目标日期的因子。\n3. 训练模型并生成研究信号。\n4. 使用真实模型结果进行历史回测。")
    research_disclaimer(st)
...
if not dsn:
    page_header(st, "量化研究工作台", "配置数据连接后开始研究。")
    empty_state(st, "尚未连接研究数据库", "页面只读取 PostgreSQL 中已经同步的数据，不会在刷新时自动请求外部接口。", "请在部署环境配置 DATABASE_URL；连接方式见 README。")
    with st.expander("高级数据连接设置"):
        st.caption("当前部署未提供 DATABASE_URL。请通过 Docker Compose 或运行环境变量配置连接串，避免在页面中保存凭据。")
    research_disclaimer(st)
    st.stop()
try:
...
page = st.sidebar.radio("研究模块", ["今日机会", "股票池", "个股研究", "行业景气", "策略实验室", "组合", "回测", "数据状态"], index=0)
