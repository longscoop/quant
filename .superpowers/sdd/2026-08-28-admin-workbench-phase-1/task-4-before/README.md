...
- Streamlit 页面：总览、数据管理、个股数据、因子研究、模型训练、策略配置、回测、运行记录。
...
```bash
cp .env.example .env
# 在 .env 填写 TUSHARE_TOKEN，并按需修改 POSTGRES_PASSWORD
docker compose up --build
```

打开 `http://localhost:8501`，在侧栏填写 `DATABASE_URL`（Compose 默认值：`postgresql+psycopg://quant:quant@postgres:5432/quant`）。首次在“数据管理”同步数据。
