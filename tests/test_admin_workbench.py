...
class AdminWorkbenchTests(unittest.TestCase):
    def test_navigation_pages_and_database_copy_are_audience_specific(self):
        ...
        public_pages = ["研究首页", "候选池", "个股详情", "行业观察", "我的组合", "历史验证", "数据状态"]
        ...
        self.assertEqual(
            ...
            ("研究数据正在准备", "当前暂时无法加载研究数据。", "请稍后刷新或联系管理员。"),
        )
        self.assertEqual(
    ...
    def test_public_data_status_uses_reader_terms_without_management_controls_or_run_ids(self):
        """Public data status must remain a read-only research-readiness view."""
        ...
        self.assertIn("研究准备", rendered)

    def test_public_status_uses_the_first_newest_run_for_each_reader_stage(self):
        """Run lists are newest-first, so later entries must not overwrite a current reader status."""
        ...
        self.assertEqual(
            next((row for row in rows if row["阶段"] == "研究准备"), None),
            {"阶段": "研究准备", "状态": "可用"},
        )
