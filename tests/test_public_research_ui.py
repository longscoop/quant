...
class PublicResearchHomeTests(unittest.TestCase):
    ...
    def test_detail_action_consumes_the_target_and_opens_the_selected_security(self):
        ...
        self.assertEqual(len(at.exception), 0)
        self.assertEqual(at.title[0].value, "个股详情")
    ...
    def test_all_candidates_action_consumes_the_target_and_opens_the_current_pool(self):
        ...
        self.assertEqual(len(at.exception), 0)
        self.assertEqual(at.title[0].value, "候选池")
        self.assertNotIn("research_target_page", at.session_state)
    ...
    def test_stock_detail_score_display_limits_visual_precision(self):
        """Rendering raw factor precision would make compact summary cards hard to scan."""
        from quant.research_ui import _display_score

        self.assertEqual(_display_score(65.6732), "65.7 / 100")
        self.assertEqual(_display_score(85.0), "85 / 100")
    ...
    def test_candidate_pool_detail_action_opens_the_selected_security(self):
        ...
        at = self._routing_app()
        at.sidebar.radio(key="research_navigation_page").set_value("候选池").run(timeout=10)
    ...
    def test_stock_detail_direct_visit_uses_dashboard_summary_and_research_sections(self):
        """Reverting the redesign would bury trust, factor, valuation, and financial evidence in one long page."""
        ...
        self.assertEqual(
            [(item.label, item.value) for item in at.metric[:6]],
            [
                ("行业", "金融服务"),
                ("研究结论", "优先研究"),
                ("因子综合分", "85 / 100"),
        ...
        self.assertEqual(
            [item.value for item in at.subheader[:7]],
            [
                "研究摘要",
                "因子评分（0-100）",
                "相对沪深300走势",
        ...
        self.assertIn("研究日期：2026-08-27", rendered)
        self.assertIn("模型日期：2026-08-27", rendered)
        self.assertIn("因子日期：2026-08-27", rendered)
        self.assertIn("核心优势", rendered)
    ...
    def test_stock_detail_replaces_valuation_rows_with_selectable_trend(self):
        """Restoring the daily valuation dataframe would hide the requested visual comparison."""
        at = self._stock_detail_app()
        metric_key = "research_detail_valuation_metric"

        self.assertEqual(len(at.exception), 0)
        self.assertIn(metric_key, [item.key for item in at.segmented_control])
        if metric_key not in [item.key for item in at.segmented_control]:
            return
        self.assertEqual(at.segmented_control(key=metric_key).value, "市盈率")
        self.assertIn("估值走势", self._rendered(at))
        self.assertNotIn("估值原始值", self._rendered(at))
        self.assertEqual(len(at.get("vega_lite_chart")), 2)

    def test_stock_detail_stacks_trend_cards_and_formats_comparison_axes(self):
        """Putting both trends in narrow columns would make comparison dates and values hard to read."""
        at = self._stock_detail_app()

        self.assertEqual(len(at.exception), 0)
        trend_titles = {"相对沪深300走势", "估值走势"}
        for column in at.get("column"):
            self.assertTrue(
                trend_titles.isdisjoint({item.value for item in column.subheader}),
                "trend cards must render as separate full-width rows",
            )

        comparison_proto = at.get("vega_lite_chart")[0].proto
        comparison_spec = json.loads(comparison_proto.spec)
        dataset_name = comparison_spec["data"]["name"]
        dataset = next(item for item in comparison_proto.datasets if item.name == dataset_name)
        comparison_data = pa.ipc.open_stream(dataset.data.data).read_all()
        self.assertTrue(pa.types.is_timestamp(comparison_data.schema.field("日期").type))
        self.assertEqual(
            set(comparison_data.column("系列").to_pylist()),
            {"个股（归一化）", "沪深300（归一化）"},
        )
        self.assertTrue(pa.types.is_floating(comparison_data.schema.field("归一化数值").type))

        self.assertEqual(comparison_spec["height"], 420)
        self.assertEqual(comparison_spec["encoding"]["x"]["axis"]["format"], "%Y-%m-%d")
        self.assertEqual(comparison_spec["encoding"]["x"]["axis"]["labelAngle"], -30)
        self.assertEqual(
            comparison_spec["encoding"]["x"]["axis"]["values"],
            ["2026-08-25T00:00:00", "2026-08-26T00:00:00", "2026-08-27T00:00:00"],
        )
        self.assertEqual(comparison_spec["encoding"]["y"]["axis"]["format"], ".2f")
        self.assertEqual(
            comparison_spec["encoding"]["tooltip"],
            [
                {"field": "日期", "format": "%Y-%m-%d", "title": "日期", "type": "temporal"},
                {"field": "系列", "title": "系列", "type": "nominal"},
                {"field": "归一化数值", "format": ".3f", "title": "数值", "type": "quantitative"},
            ],
        )

    def test_stock_detail_valuation_trend_switches_metric(self):
        """Ignoring the selected valuation metric would make the chart selector a no-op."""
        at = self._stock_detail_app()
        metric_key = "research_detail_valuation_metric"
        ...
        if metric_key not in [item.key for item in at.segmented_control]:
            return
        at.segmented_control(key=metric_key).set_value("市净率").run(timeout=10)

        self.assertEqual(len(at.exception), 0)
        self.assertEqual(at.segmented_control(key=metric_key).value, "市净率")
        self.assertIn("当前指标：市净率（倍）", self._rendered(at))
    ...
    def test_stock_detail_does_not_render_duplicate_portfolio_controls(self):
        """Restoring a detail-page portfolio card would duplicate the standalone portfolio workbench."""
        ...
        self.assertFalse(
            set(labels)
            & {"加入研究组合", "已加入", "检查我的组合", "保存目标权重"}
        )
        rendered = self._rendered(at)
        self.assertNotIn("我的研究组合", rendered)
        self.assertNotIn("当前目标权重", rendered)

    def test_candidate_detail_journey_stops_at_read_only_stock_detail(self):
        """Adding portfolio controls to the detail page would recreate the duplicate research-combination entry."""
        at = self._stock_detail_app()
        at.sidebar.radio(key="research_navigation_page").set_value("研究首页").run(timeout=10)
        ...
        self.assertEqual(at.title[0].value, "个股详情")
        self.assertEqual(at.selectbox(key="research_detail_security").value, "000001.SZ")
        self.assertNotIn("research_detail_add_000001.SZ", [button.key for button in at.button])
        self.assertNotIn("research_detail_check_portfolio_000001.SZ", [button.key for button in at.button])
