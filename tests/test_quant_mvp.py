...
class QuantMvpTests(unittest.TestCase):
    ...
    def test_legacy_factor_features_use_latest_nonfuture_valuation_bar(self):
        """The legacy path must use daily valuation bars rather than financial-record copies."""
        self.store.valuations[("000001.SZ", date(2024, 4, 15))] = ValuationBar(
            "000001.SZ", date(2024, 4, 15), 8.0, 1.2, 0.9, 0.03
        )
        self.store.valuations[("000001.SZ", date(2024, 4, 16))] = ValuationBar(
            "000001.SZ", date(2024, 4, 16), 99.0, 9.9, 9.9, 0.99
        )

        rows = FactorEngine(PITRepository(self.store)).build_features([date(2024, 4, 15)]).rows
        row = next(item for item in rows if item.ts_code == "000001.SZ")

        self.assertGreater(row.values["value_pe"], 0.0)

    def test_legacy_model_does_not_fabricate_predictions_without_labels(self):
        engine = FactorEngine(PITRepository(self.store))
        ...
        self.assertFalse(prediction.rows)
        self.assertEqual(prediction.metadata["status_reason"], "missing_labels")
    ...
    def test_public_run_presentation_hides_run_ids_and_stored_failures(self):
        ...
        self.assertEqual(public_run_label(failed), "研究结果 · 失败")
        self.assertEqual(
            public_run_feedback(failed, "模型研究已完成。"),
            ("error", "研究结果未能完成。请稍后重试或联系管理员。"),
        )
        self.assertEqual(
            public_run_feedback(completed, "历史验证已完成。"),
            ("success", "历史验证已完成。"),
        )
    ...
    def test_streamlit_entrypoint_gates_navigation_and_database_copy_by_audience(self):
        ...
        self.assertEqual(
            ...
            ["研究首页", "候选池", "个股详情", "行业观察", "我的组合", "历史验证", "数据状态"],
        )
        public_output = "\n".join(
        ...
        self.assertEqual(
            ...
            ["研究首页", "候选池", "个股详情", "行业观察", "我的组合", "历史验证", "数据状态", "管理员"],
        )
        admin_output = "\n".join(
    ...
    def test_status_feedback_warns_for_not_trainable_model_and_succeeds_only_when_completed(self):
        ...
        self.assertEqual(
            ...
            ("error", "研究准备未能完成。请稍后重试或联系管理员。"),
        )

    def test_model_does_not_fall_back_when_lightgbm_shared_library_cannot_load(self):
        engine = FactorEngine(PITRepository(self.store))
        features = engine.build_features([date(2024, 3, 15), date(2024, 4, 15), date(2024, 5, 15)])
        ...
        with patch("builtins.__import__", side_effect=broken_lightgbm_import):
            labels = {(row.as_of_date, row.ts_code): float(index) for index, row in enumerate(features.rows)}
            prediction = ModelTrainer().fit_predict(features, [date(2024, 5, 15)], labels=labels)
        self.assertFalse(prediction.rows)
        self.assertIn(prediction.metadata["status_reason"], {"insufficient_training_samples", "lightgbm_unavailable"})
