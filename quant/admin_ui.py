...
from .markets import DateSegment, TimeSplitConfig
...
def _render_model_form(st, store, can_run: bool, runs: list[dict]) -> None:
    ...
    with st.form("admin_model"):
        st.caption("使用所选的已完成因子运行。")
        st.caption("CN 首版使用完整市场上下文；训练、验证、测试均为按交易日顺序的显式区间。")
        available_dates = _factor_run_dates(factor_run)
        if len(available_dates) < 3:
            available_dates = _trading_dates_through(store, default_prediction)
        defaults = _default_model_segments(available_dates, default_prediction)
        train_start = _date_input(st, "训练开始", defaults[0], "admin_model_train_start")
        train_end = _date_input(st, "训练结束", defaults[1], "admin_model_train_end")
        valid_start = _date_input(st, "验证开始", defaults[2], "admin_model_valid_start")
        valid_end = _date_input(st, "验证结束", defaults[3], "admin_model_valid_end")
        test_start = _date_input(st, "测试开始", defaults[4], "admin_model_test_start")
        test_end = _date_input(st, "测试结束", defaults[5], "admin_model_test_end")
        st.caption(
            f"市场 {CN_DEFAULT_CONTEXT.market_id} · 币种 {CN_DEFAULT_CONTEXT.currency} · "
            f"日历 {CN_DEFAULT_CONTEXT.calendar_id} · 股票池 {CN_DEFAULT_CONTEXT.universe_id} · "
            f"基准 {CN_DEFAULT_CONTEXT.benchmark_id}"
        )
        submitted = st.form_submit_button(
            "生成研究模型",
...
def _factor_run_dates(run: dict | None) -> list[date]:
    if not run:
        return []
    return sorted({
        value
        for row in (run.get("payload") or {}).get("rows") or []
        if (value := _coerce_date(row.get("as_of_date"))) is not None
    })


def _default_model_segments(available_dates: list[date], fallback: date) -> tuple[date, date, date, date, date, date]:
    if len(available_dates) >= 3:
        valid_index = max(1, int(len(available_dates) * 0.6))
        test_index = max(valid_index + 1, int(len(available_dates) * 0.8))
        test_index = min(test_index, len(available_dates) - 1)
        return (
            available_dates[0],
            available_dates[valid_index - 1],
            available_dates[valid_index],
            available_dates[test_index - 1],
            available_dates[test_index],
            available_dates[-1],
        )
